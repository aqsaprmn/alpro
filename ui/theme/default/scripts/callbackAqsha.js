const host = "http://localhost/alpro/index.php?_route=datacall/";
// const host = "http://103.242.181.10/alpro/index.php?_route=datacall/";
            ...
            if (alpha.includes(keyPress)) {
              ...
              if (val != "" || val != null) {
                if (!loader.classList.contains("dnone")) {
                  ...
                  xhttp.open("GET", host + idInputDivIn + "/" + val);
...
for (let j = 0; j < divInput.length; j++) {
  ...
  if (dataSelect) {
    ...
    const divInputFormIn = form.querySelectorAll(
    ...
    for (let k = 0; k < divInputFormIn.length; k++) {
      ...
      if (dataTable) {
        ...
        if (valInputDiv != "") {
          fetch(host + idSelect + "/" + valInputDiv + "/" + idodp)
        ...
        inputDivIn.addEventListener("focusout", function () {
          ...
          if (valInputDivIn != "") {
            fetch(host + idSelect + "/" + valInputDivIn)
