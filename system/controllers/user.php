...
switch ($action) {
    ...
    case 'users-activate':
        ...
        $id  = $routes['2'];
        if (($admin['id']) == $id) {
            r2(U . 'settings/users', 'e', 'Sorry You can\'t activate yourself');
        }
        $d = ORM::for_table('tbl_users')->find_one($id);
        if ($d) {
            $d->status = 'Active';
            $d->save();
            _log('[' . $admin['username'] . ']: ' . 'User Is Activated Successfully', 'Admin', $admin['id']);
            $harian = $d->harian;
            $existing_harian = ORM::for_table('tbl_voucher')->where('id_agen', $id)->where('id_plan', 2)->where('status', '0')->count();
            //r2(U.'prepaid/voucher-refill/'.$id, 'e', $existing_harian.'-');
            $harian_to_update = $harian - $existing_harian;
            if ($harian_to_update > 0) {
                $dd1 = ORM::for_table('tbl_voucher')->where('id_agen', 0)->where('id_plan', 2)->order_by_asc('id')->limit($harian_to_update)->find_many();
                if ($dd1) {
                    foreach ($dd1 as $row1) {
                        $row1->id_agen = $id;
                        $row1->save();
                ...
                $dd2 = ORM::for_table('tbl_voucher')->where('id_agen', $id)->where('id_plan', 2)->where('status', '1')->find_many();
                if ($dd2) {
                    foreach ($dd2 as $row2) {
                        $row2->status = '2';
                        $row2->shown = '1';
            ...
            $mingguan = $d->mingguan;
            $existing_mingguan = ORM::for_table('tbl_voucher')->where('id_agen', $id)->where('id_plan', 3)->where('status', '0')->count();
            //r2(U.'prepaid/voucher-refill/'.$id, 'e', $existing_mingguan.'-');
            $mingguan_to_update = $mingguan - $existing_mingguan;
            if ($mingguan_to_update > 0) {
                $dd1 = ORM::for_table('tbl_voucher')->where('id_agen', 0)->where('id_plan', 3)->order_by_asc('id')->limit($mingguan_to_update)->find_many();
                if ($dd1) {
                    foreach ($dd1 as $row1) {
                        $row1->id_agen = $id;
                        $row1->save();
                ...
                $dd2 = ORM::for_table('tbl_voucher')->where('id_agen', $id)->where('id_plan', 3)->where('status', '1')->find_many();
                if ($dd2) {
                    foreach ($dd2 as $row2) {
                        $row2->status = '2';
                        $row2->shown = '1';
            ...
            $bulanan = $d->bulanan;
            $existing_bulanan = ORM::for_table('tbl_voucher')->where('id_agen', $id)->where('id_plan', 4)->where('status', '0')->count();
            //r2(U.'prepaid/voucher-refill/'.$id, 'e', $existing_bulanan.'-');
            $bulanan_to_update = $bulanan - $existing_bulanan;
            if ($bulanan_to_update > 0) {
                $dd1 = ORM::for_table('tbl_voucher')->where('id_agen', 0)->where('id_plan', 4)->order_by_asc('id')->limit($bulanan_to_update)->find_many();
                if ($dd1) {
                    foreach ($dd1 as $row1) {
                        $row1->id_agen = $id;
                        $row1->save();
                ...
                $dd2 = ORM::for_table('tbl_voucher')->where('id_agen', $id)->where('id_plan', 4)->where('status', '1')->find_many();
                if ($dd2) {
                    foreach ($dd2 as $row2) {
                        $row2->status = '2';
                        $row2->shown = '1';
            ...
            r2(U . 'settings/users', 's', 'User is activated');
        } else {
            r2(U . 'settings/users', 'e', $_L['Account_Not_Found']);
        }
        break;
    case 'users-deactivate':
        ...
        $id  = $routes['2'];
        if (($admin['id']) == $id) {
            r2(U . 'settings/users', 'e', 'Sorry You can\'t deactivate yourself');
        }
        $d = ORM::for_table('tbl_users')->find_one($id);
        if ($d) {
            $d->status = 'Inactive';
            $d->save();
            _log('[' . $admin['username'] . ']: ' . 'User Is Deactivated Successfully', 'Admin', $admin['id']);
            r2(U . 'settings/users', 's', 'User is deactivated');
        } else {
            r2(U . 'settings/users', 'e', $_L['Account_Not_Found']);
        }
        break;
