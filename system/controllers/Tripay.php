...
class Tripay {
	//sandbox keys
	//protected $api_PKey = 'vbnhG-2Y3YA-TGslR-BDQcS-BXkmR';
	//protected $api_Key = 'DEV-TPUfjAledP6DTePwcynHfzOUWrNhMKSPVIXClugz';
	//production keys
	protected $api_PKey = 'egm0P-EWMh3-wl5Ml-3Oexb-EZqBI';
	protected $api_Key = 'q5rMVQuOcnbNbKHCMuvvzlcMgDHIr6tYpJObH0Fv';
	// Database
	protected $dbH;
	...
	*/
	...
	public function __construct($privateKey = null,$apiKey = null){
		if ($privateKey === null AND $apiKey === null) {
			...
			$this->api_Key = $apiKey;
			$this->dbH = new mysqli("localhost", "root", "", "broiler");
		}
	}
	...
	public function callBack(){
		...
		$callbackSignature = isset($_SERVER['HTTP-X-CALLBACK-SIGNATURE']) ? $_SERVER['HTTP-X-CALLBACK_SIGNATURE'] : '';
		...
		if($callbackSignature != $signature) {
			...
			//echo $_SERVER['HTTP-X-CALLBACK-SIGNATURE'];
		    exit('Signature tidak valid'); // signature tidak valid, hentikan proses
		} else {
			// Data
			$data = json_decode($json);
			$event = $_SERVER['HTTP-X-CALLBACK_EVENT'];
